document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── Scroll reveal (slide from sides/top) ── */
  ...
  revealEls.forEach(el => slideObserver.observe(el));

  /* ── Counter animation for numeric stats only ── */
  ...
  const counterObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        if (!/^[\d,+.]+$/.test(raw)) { counterObserver.unobserve(el); return; }
